...
def apply_tf_idf_to_sentences(X_train, X_test):
...
def evaluate_model_on_test_set(X_test_tfidf, Y_test, model):
...
if __name__ == "__main__":
    ...
    X_test, Y_test = load_test_data(test_dataset_path)

    # Apply TF-IDF to both train and test sets
    X_train_tfidf, X_test_tfidf = apply_tf_idf_to_sentences(X_train, X_test)

    # Train the model
    trained_svm_model = train_model(X_train_tfidf, Y_train)

    # Evaluate the model
    evaluate_model_on_test_set(X_test_tfidf, Y_test, trained_svm_model)
